...
class Html5Qrcode {
  ...
  constructor(elementId, verbose) {
    if (!getLazarSoftScanner) {
      throw (
        "Use html5qrcode.min.js without edit, getLazarSoftScanner" +
        "not found."
      );
    }
    ...
    if (!this.qrcode) {
      throw (
        "qrcode is not defined, use the minified/html5-qrcode.min.js" +
        " for proper support"
      );
    }
  ...
  start(cameraId, configuration, qrCodeSuccessCallback, qrCodeErrorCallback) {
    ...
    if (isShadedBoxEnabled) {
      ...
      if (qrboxSize > width) {
        throw (
          "'config.qrbox' should not be greater than the " +
    ...
    const setupUi = (width, height) => {
      ...
      if (qrboxSize > height) {
        console.warn(
          "[Html5Qrcode] config.qrboxsize is greater " +
    ...
    const foreverScan = () => {
      ...
      $this._foreverScanTimeout = setTimeout(
        foreverScan,
        Html5Qrcode._getTimeoutFps(config.fps)
      );
    };
      ...
      const removeQrRegion = () => {
        while (
          $this._element.getElementsByClassName(
            Html5Qrcode.SHADED_REGION_CLASSNAME
          ).length
        ) {
  ...
  scanFile(imageFile, /* default=true */ showImage) {
    ...
    if (!imageFile || !(imageFile instanceof File)) {
      throw (
        "imageFile argument is mandatory and should be instance " +
    ...
    return new Promise((resolve, reject) => {
      ...
      inputImage.onload = () => {
        ...
        const hiddenCanvas = $this._createCanvasElement(
          config.width,
          config.height
        );
