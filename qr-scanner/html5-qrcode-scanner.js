...
class Html5QrcodeScanner {
    ...
    this.qrCodeSuccessCallback = (message) => {
      ...
      if (qrCodeSuccessCallback) {
        ...
        $this.__setHeaderMessage(
          `Last Match: ${message}`,
          Html5QrcodeScanner.STATUS_SUCCESS
        );
      }
    };
  ...
  __renderCameraSelection(cameras) {
    ...
    cameraActionStartButton.addEventListener("click", (_) => {
      ...
      const config = $this.config ? $this.config : { fps: 10, qrbox: 250 };
  ...
  __createSectionSwap() {
    const $this = this;
    const TEXT_IF_CAMERA_SCAN_SELECTED = "Scan an Image File";
    const TEXT_IF_FILE_SCAN_SELECTED = "Scan using camera directly";
  ...
  _showHideScanTypeSwapLink(shouldDisplay) {
    ...
    this.__getDashboardSectionSwapLink().style.display = shouldDisplay
      ? "inline-block"
      : "none";
  }
